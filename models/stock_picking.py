...
class StockMove(models.Model):
    ...
    def _compute_amount(self):
        for rec in self:
            factor = rec.product_id.product_tmpl_id.uom_venta_id.factor
            qty = round(rec.product_uom_qty / (1 / factor))
    ...
    total_unidades_venta  = fields.Float(string="Saldo", readonly=True, store=True,
                               compute='_calcular_total_unidades_venta')

    @api.onchange('name', 'product_id', 'move_line_tax_ids', 'product_uom_qty', 'precio_unitario', 'quantity_done')
    @api.depends('name', 'product_id', 'move_line_tax_ids', 'product_uom_qty', 'precio_unitario', 'quantity_done')
    def _calcular_total_unidades_venta(self):
        for rec in self:
           factor = rec.product_id.product_tmpl_id.uom_venta_id.factor
        rec.total_unidades_venta = round(rec.product_uom_qty / (1 / factor))
